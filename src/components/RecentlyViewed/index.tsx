...
const CardContent: React.FC<Props> = ({ item }) => {
  return (
    <Card
      key={item.id}
      className="lg:w-80 px-2 text-text my-3 mx-4 shadow-none"
    >
      <Box className="relative overflow-hidden">
        <Image
          alt={item.title}
          src={item.image}
          className="w-full h-56 rounded-sm object-cover transition duration-500 transform hover:scale-105"
        />
        <Paragraph
